...
public class CourseMain {
	public static void main(String[] args) throws FileNotFoundException, IOException, NotBoundException {
		...
		while (!done) {
			...
			for (int i = 0; i < eventQueue.getSize(); i++) {
				...
				case SignupCourses:
					printLogEvent("Get", event);
					eventBus.sendEvent(new Event(EventId.SignupStudents, signupCourse(coursesList, event.getMessage())));
					break;
				case QuitTheSystem:
	...
	private static String deleteCourse(CourseComponent coursesList, String message) {
		if (coursesList.isRegisteredCourse(message) == true) {
			coursesList.vCourse.remove(coursesList.indexCourse(message));
			return "This course is successfully delete.";
		} else {
			return "This course is unregistered.";
		}
	}
	private static String signupCourse(CourseComponent coursesList, String message) {
		String result = coursesList.getCourseInfo(message.split(" ")[1]);
		if (result.equals("-1")) {return message + " " + "-1";}
		else { return message + " " + coursesList.getCourseInfo(message.split(" ")[1]);}
	}
